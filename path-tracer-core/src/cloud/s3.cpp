...
namespace cloud
{
void s3_download_object(const std::string &bucket, const std::string &key,
...
void s3_upload_object(const std::string &bucket, const std::string &key,
                      std::variant<std::filesystem::path, std::vector<uint8_t>> &input)
{
    ...
    if (std::holds_alternative<std::filesystem::path>(input))
    {
        ...
        std::shared_ptr<Aws::IOStream> file_stream = Aws::MakeShared<Aws::FStream>(
            "PutObjectInputStream", file_path.c_str(), std::ios_base::in | std::ios_base::binary);
    ...
    else
    {
        ...
        auto data = Aws::MakeShared<Aws::StringStream>(
            "PutObjectInputStream", std::stringstream::in | std::stringstream::out | std::stringstream::binary);

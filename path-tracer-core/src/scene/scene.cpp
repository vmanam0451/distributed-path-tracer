...
namespace cloud {
    ...
    void distributed_scene::load_scene(const std::filesystem::path& gltf_path) {
		...
		if (sun_light_index != cloud::distributed_scene::no_sun_light) {
    ...
    std::shared_ptr<core::mesh> distributed_scene::get_mesh(cgltf_primitive* primitive, const std::filesystem::path& gltf_path) {
		...
		for (int i = 0; i < primitive->attributes_count; i++) {
			...
			if (!buffer_loaded) {
                ...
                cloud::s3_download_object(this->scene_s3_bucket, buffer_uri, output);
	...
	std::shared_ptr<core::material> distributed_scene::get_material(cgltf_primitive* primitive) {
		...
		if (!cgltf_normal_tex_path.empty()) {
			auto normal_tex = get_cached_texture(this->scene_s3_bucket, cgltf_normal_tex_path, false);
			material->normal_tex = normal_tex;
		}

		if (!cgltf_albedo_opacity_tex_path.empty()) {
			auto albedo_opacity_tex = get_cached_texture(this->scene_s3_bucket, cgltf_albedo_opacity_tex_path, true);
		...
		if (!cgltf_occlusion_tex_path.empty()) {
			auto occlusion_tex = get_cached_texture(this->scene_s3_bucket, cgltf_occlusion_tex_path, false);
			material->occlusion_tex = occlusion_tex;
		}

		if (!cgltf_roughness_metallic_tex_path.empty()) {
			auto roughness_metallic_tex = get_cached_texture(this->scene_s3_bucket, cgltf_roughness_metallic_tex_path,
				false);
		...
		if (!cgltf_emissive_tex_path.empty()) {
			auto emissive_tex = get_cached_texture(this->scene_s3_bucket, cgltf_emissive_tex_path, true);

...
namespace cloud {
void distributed_scene::load_scene(
    const std::string& scene_s3_bucket, const std::string& scene_s3_root,
  ...
  cgltf_result result =
      cgltf_parse_file(&options, gltf_path.string().c_str(), &m_data);
  ...
  if (m_data->cameras_count < camera_index + 1)
    throw std::runtime_error("Scene does not contain camera #" +
                             util::to_string(camera_index) + ".");
  ...
  if (sun_light_index != no_sun_light) {
    if (m_data->lights_count < sun_light_index + 1) {
      spdlog::error(
          "Scene does not contain sun light #{} No sun light will be used.",
          sun_light_index);
    } else if (m_data->lights[sun_light_index].type !=
               cgltf_light_type_directional) {
      spdlog::error("Light #{} is not a sun light. No sun light will be used.",
                    sun_light_index);
    } else {
  ...
  for (int i = 0; i < main_scene.nodes_count; i++) {
    process_node(main_scene.nodes[i], cgltf_camera, cgltf_sun_light, nullptr,
                 gltf_path);
  }

  if (!m_camera) throw std::runtime_error("Scene is missing a camera.");
...
void distributed_scene::process_node(cgltf_node* cgltf_node,
                                     cgltf_camera* cgltf_camera,
                                     cgltf_light* cgltf_sun_light,
                                     scene::entity* parent,
                                     const std::filesystem::path& gltf_path) {
  // Set Properties
  ...
  cgltf_float* cgltf_rotation =
      cgltf_node->has_rotation ? cgltf_node->rotation : nullptr;
  math::quat rotation;
  if (cgltf_rotation)
    rotation = math::quat(cgltf_rotation[3], cgltf_rotation[0],
                          cgltf_rotation[1], cgltf_rotation[2]);

  cgltf_float* cgltf_scale =
      cgltf_node->has_scale ? cgltf_node->scale : nullptr;
  math::fvec3 scale =
      cgltf_scale ? math::fvec3(cgltf_scale[0], cgltf_scale[1], cgltf_scale[2])
                  : math::fvec3::one;

  cgltf_float* cgltf_translation =
      cgltf_node->has_translation ? cgltf_node->translation : nullptr;
  math::fvec3 translation =
      cgltf_translation
          ? math::fvec3(cgltf_translation[0], cgltf_translation[1],
                        cgltf_translation[2])
          : math::fvec3::zero;

  entity->set_local_transform(
      scene::transform::make(translation, rotation, scale));
  ...
  if (cgltf_node->mesh) {
    ...
    for (uint32_t i = 0; i < cgltf_node->mesh->primitives_count; i++) {
      if (std::find(model_primitives.begin(), model_primitives.end(), i) ==
          model_primitives.end()) {
        continue;
      }
  ...
  if (cgltf_sun_light &&
      entity->get_name() == std::string(cgltf_sun_light->name)) {
    auto sun_light = entity->add_component<scene::sun_light>();

    this->m_sun_light = entity;

    sun_light->energy =
        math::fvec3(cgltf_sun_light->color[0], cgltf_sun_light->color[1],
                    cgltf_sun_light->color[2]) *
        cgltf_sun_light->intensity;
  }

  if (cgltf_node->children_count > 0) {
    for (int i = 0; i < cgltf_node->children_count; i++)
      process_node(cgltf_node->children[i], cgltf_camera, cgltf_sun_light,
                   entity.get(), gltf_path);
  }
...
std::shared_ptr<image::texture> distributed_scene::get_cached_texture(
    const std::string& scene_bucket, const std::string& image_key, bool srgb) {
  if (m_texture_cache.contains(image_key)) {
    auto texture = m_texture_cache[image_key].lock();
    if (texture) return texture;
  }

  std::variant<std::filesystem::path, std::vector<uint8_t>> output{
      std::vector<uint8_t>{}};
  cloud::s3_download_object(scene_bucket, image_key, output);

  auto texture =
      image::image_texture::load_from_memory(std::get<1>(output), srgb);
...
std::shared_ptr<core::mesh> distributed_scene::get_mesh(
    cgltf_primitive* primitive, const std::filesystem::path& gltf_path) {
  ...
  for (int i = 0; i < primitive->attributes_count; i++) {
    ...
    if (!buffer_loaded) {
      ...
      cloud::s3_download_object(this->m_scene_s3_bucket,
                                this->m_scene_s3_root + buffer_uri, output);

      cgltf_options options = {};
      cgltf_load_buffer_file(&options, buffer->size, buffer_uri.c_str(),
                             gltf_path.string().c_str(), &buffer->data);
    }
  ...
  cgltf_accessor_unpack_indices(primitive->indices, indices.data(),
                                sizeof(unsigned int),
  ...
  for (size_t i = 0, j = 0, k = 0; k < vertices_size;
       i += VEC_3_STRIDE, j += VEC_2_STRIDE, k++) {
...
std::shared_ptr<core::material> distributed_scene::get_material(
  ...
  cgltf_float* cgltf_albedo_opacity =
      cgltf_material->pbr_metallic_roughness.base_color_factor;  // float[4]
  cgltf_float cgltf_roughness =
      cgltf_material->pbr_metallic_roughness.roughness_factor;
  cgltf_float cgltf_metallic =
      cgltf_material->pbr_metallic_roughness.metallic_factor;
  ...
  auto cgltf_albedo_opacity_texture =
      cgltf_material->pbr_metallic_roughness.base_color_texture;
  auto cgltf_occlusion_texture = cgltf_material->occlusion_texture;
  auto cgltf_roughness_metallic_texture =
      cgltf_material->pbr_metallic_roughness.metallic_roughness_texture;
  auto cgltf_emissive_texture = cgltf_material->emissive_texture;

  std::string cgltf_normal_tex_path =
      cgltf_normal_texture.texture ? cgltf_normal_texture.texture->image->uri
                                   : "";
  std::string cgltf_albedo_opacity_tex_path =
      cgltf_albedo_opacity_texture.texture
          ? cgltf_albedo_opacity_texture.texture->image->uri
          : "";
  std::string cgltf_occlusion_tex_path =
      cgltf_occlusion_texture.texture
          ? cgltf_occlusion_texture.texture->image->uri
          : "";
  std::string cgltf_roughness_metallic_tex_path =
      cgltf_roughness_metallic_texture.texture
          ? cgltf_roughness_metallic_texture.texture->image->uri
          : "";
  std::string cgltf_emissive_tex_path =
      cgltf_emissive_texture.texture
          ? cgltf_emissive_texture.texture->image->uri
          : "";

  material->albedo_fac =
      math::fvec3(cgltf_albedo_opacity[0], cgltf_albedo_opacity[1],
                  cgltf_albedo_opacity[2]);
  material->opacity_fac = cgltf_albedo_opacity[3];
  material->roughness_fac = cgltf_roughness;
  material->metallic_fac = cgltf_metallic;
  material->emissive_fac =
      math::fvec3(cgltf_emissive[0], cgltf_emissive[1], cgltf_emissive[2]);

  if (!cgltf_normal_tex_path.empty()) {
    auto normal_tex = get_cached_texture(
        this->m_scene_s3_bucket, this->m_scene_s3_root + cgltf_normal_tex_path,
        false);
    material->normal_tex = normal_tex;
  }

  if (!cgltf_albedo_opacity_tex_path.empty()) {
    auto albedo_opacity_tex = get_cached_texture(
        this->m_scene_s3_bucket,
        this->m_scene_s3_root + cgltf_albedo_opacity_tex_path, true);
  ...
  if (!cgltf_occlusion_tex_path.empty()) {
    auto occlusion_tex = get_cached_texture(
        this->m_scene_s3_bucket,
        this->m_scene_s3_root + cgltf_occlusion_tex_path, false);
  ...
  if (!cgltf_roughness_metallic_tex_path.empty()) {
    auto roughness_metallic_tex = get_cached_texture(
        this->m_scene_s3_bucket,
        this->m_scene_s3_root + cgltf_roughness_metallic_tex_path, false);
  ...
  if (!cgltf_emissive_tex_path.empty()) {
    auto emissive_tex = get_cached_texture(
        this->m_scene_s3_bucket,
        this->m_scene_s3_root + cgltf_emissive_tex_path, true);
    material->emissive_tex = emissive_tex;
  }

  std::string name{cgltf_material->name};
  if (name.find("shadow") != std::string::npos &&
      name.find("catcher") != std::string::npos)

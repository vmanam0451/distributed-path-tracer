...
namespace cloud {
models::intersect_result_min distributed_scene::intersect_min_result(
  ...
  while (!stack.empty()) {
    ...
    for (const auto& child : entity->get_children()) stack.push(child.get());
    ...
    if (auto model = entity->get_component<scene::model>()) {
      ...
      if (hit.distance < nearest_hit.distance || !nearest_hit.has_hit()) {
        nearest_hit = hit;
      }
    }
  }

  if (!nearest_hit.has_hit()) return {false, std::numeric_limits<float>::max()};
  ...
  fvec3 position = transform * (v1.position * nearest_hit.barycentric.x +
                                v2.position * nearest_hit.barycentric.y +
                                v3.position * nearest_hit.barycentric.z);

  fvec2 tex_coord = v1.tex_coord * nearest_hit.barycentric.x +
                    v2.tex_coord * nearest_hit.barycentric.y +
                    v3.tex_coord * nearest_hit.barycentric.z;

  fvec3 normal =
      normalize(normal_matrix * (v1.normal * nearest_hit.barycentric.x +
                                 v2.normal * nearest_hit.barycentric.y +
                                 v3.normal * nearest_hit.barycentric.z));

  fvec3 tangent =
      normalize(normal_matrix * (v1.tangent * nearest_hit.barycentric.x +
...
models::intersect_result distributed_scene::intersect(
    const geometry::ray& ray) const {
  std::stack<entity*> stack;
  for (const auto& [_, entity] : m_entities) {
    stack.push(entity.get());
  ...
  if (!nearest_hit.has_hit()) return {false};
  ...
  return {true, material, position, tex_coord, normal, tangent};
}
}  // namespace cloud
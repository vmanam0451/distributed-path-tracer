...
class distributed_scene {
 public:
  void load_scene(const std::string& scene_s3_bucket,
                  const std::string& scene_s3_root,
                  const std::map<mesh_name, primitives>& scene_work,
                  const std::filesystem::path& gltf_path);
  models::intersect_result_min intersect_min_result(
      const geometry::ray& ray) const;
  models::intersect_result intersect(const geometry::ray& ray) const;

 private:
  void process_node(cgltf_node* cgltf_node, cgltf_camera* cgltf_camera,
                    cgltf_light* cgltf_sun_light, scene::entity* parent,
                    const std::filesystem::path& gltf_path);
  std::shared_ptr<core::mesh> get_mesh(cgltf_primitive* primitive,
                                       const std::filesystem::path& gltf_path);
  std::shared_ptr<core::material> get_material(cgltf_primitive* primitive);

  std::shared_ptr<image::texture> get_cached_texture(
 ...
 public:  // TODO: Change to private
  ...
  std::unordered_map<std::string, std::weak_ptr<image::texture>>
      m_texture_cache;

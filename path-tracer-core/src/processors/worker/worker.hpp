...
class worker : public application {
 ...
 private:
  ...
  math::fvec4 trace_iter(uint8_t initial_bounce,
 ...
 private:
  ...
  moodycamel::ConcurrentQueue<models::cloud_ray>
      m_object_intersection_result_queue;

  moodycamel::ConcurrentQueue<models::cloud_ray>
      m_direct_lighting_intersection_queue;
  moodycamel::ConcurrentQueue<models::cloud_ray>
      m_direct_lighting_intersection_result_queue;
  ...
  std::map<uint64_t, std::pair<int, models::cloud_ray>>
      m_object_intersection_results;
  std::map<uint64_t, std::pair<int, models::cloud_ray>>
      m_direct_lighting_intersection_results;
};
}  // namespace processors
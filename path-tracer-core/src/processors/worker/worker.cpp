...
void worker::run() {
  ...
  m_scene.load_scene(m_worker_info.scene_bucket, m_worker_info.scene_root, work,
                     m_gltf_file_path);
  ...
  unsigned int available_threads =
      hardware_threads -
      4;  // 1 for main, 1 for accumulation, 2 for debug & monitor
  ...
  for (int i = 0; i < object_intersection_result_threads; i++)
    threads.push_back(
        std::thread(&worker::process_object_intersection_results, this));

  for (int i = 0; i < direct_lighting_intersection_threads; i++)
    threads.push_back(
        std::thread(&worker::process_direct_lighting_intersections, this));
  for (int i = 0; i < direct_lighting_intersection_result_threads; i++)
    threads.push_back(std::thread(
        &worker::process_direct_lighting_intersection_results, this));
  ...
  threads.push_back(std::thread([&]() {
    while (!m_should_terminate) {
      spdlog::info(
          "Queue sizes: INTERSECT={}, INTERSECT_RESULT={}, DIRECT={}, "
          ...
          m_shading_queue.size_approx(), m_accumulate_queue.size_approx());
      spdlog::info("Completed Rays: {}", m_completed_rays.load());
      std::this_thread::sleep_for(std::chrono::seconds(1));
    }
  }));

  spdlog::info("Hardware Threads {} Total Threads {},", hardware_threads,
               threads.size());
  ...
  cloud::s3_upload_object(m_worker_info.scene_bucket,
                          m_worker_info.scene_root + "test.png", input);
}
  ...
  for (uint32_t x = 0; x < resolution.x; x++) {
    for (uint32_t y = 0; y < resolution.y; y++) {
      for (uint32_t sample = 0; sample < sample_count; sample++) {
        ...
        geometry::ray ray =
            m_scene.m_camera->get_component<scene::camera>()->get_ray(ndc,
                                                                      ratio);
...
std::vector<uint8_t> worker::render() const {
  ...
  // Claiming pixels by opaque samples is needed for proper transparent
  // background blending
    ...
    for (uint32_t y = 0; y < resolution.y; y++) {
      todo.push_back(pool.submit([&, y](uint32_t) {
        for (uint32_t x = 0; x < resolution.x; x++) {
          ...
          // Do not offset the first sample so we can get a consistent alpha
          // mask for smart blending
          fvec2 aa_offset = fvec2(core::rand(), core::rand());

          fvec2 ndc =
              ((fvec2(pixel) + aa_offset) / resolution) * 2 - fvec2::one;
          ndc.y = -ndc.y;
          float ratio = static_cast<float>(resolution.x) / resolution.y;

          ray ray = m_scene.m_camera->get_component<scene::camera>()->get_ray(
              ndc, ratio);
          ...
          if (transparent_background) {
            if (data.w > 0.5 && !pixels[x][y].claimed) {
              ...
              pixels[x][y].alpha =
                  pixels[x][y].alpha * sample + data.w;  // Blend only alpha
          ...
          // Otherwise if an opaque sample blends with a claimed pixel (or
          // transparent background is disabled)
          pixels[x][y].color =
              pixels[x][y].color * sample + fvec3(data);  // Blend color
          pixels[x][y].color /= sample + 1;
          pixels[x][y].alpha =
              pixels[x][y].alpha * sample + data.w;  // Blend alpha
          pixels[x][y].alpha /= sample + 1;
        }
      }));
    }

    for (auto& future : todo) future->wait();
...
fvec4 worker::trace_iter(uint8_t initial_bounce,
                         const geometry::ray& initial_ray) const {
  using namespace core;
  using namespace math;
  ...
  while (bounce_remaining > 0) {
    ...
    if (!result.hit) {
      if (m_scene.m_environment) {
        fvec3 env_color =
            fvec3(m_scene.m_environment->sample(
                core::equirectangular_proj(current_ray.get_dir()))) *
            environment_factor;
    ...
    if (!math::is_approx(opacity, 1) && core::rand() > opacity) {
      ...
      current_ray =
          geometry::ray(result.position + current_ray.get_dir() * math::epsilon,
                        current_ray.get_dir());
    ...
    if (result.material->shadow_catcher && bounce_remaining == initial_bounce) {
      ...
      if (m_scene.m_sun_light) {
        fvec3 direct_incoming =
            m_scene.m_sun_light->get_global_transform().basis * fvec3::backward;
        direct_incoming = util::rand_cone_vec(
            core::rand(),
            math::cos(core::rand() *
                      m_scene.m_sun_light->get_component<scene::sun_light>()
                          ->angular_radius),
            direct_incoming);
    ...
    float specular_probability =
        core::pbr::fresnel(outcoming, reflect(-outcoming, normal), ior);
    ...
    if (m_scene.m_sun_light) {
      ...
      if (math::dot(normal, direct_incoming) > 0) {
        geometry::ray direct_ray(
            result.position + direct_incoming * math::epsilon, direct_incoming);
        ...
        if (!direct_result.hit) {
          ...
          float specular_pdf =
              pbr::pdf_specular(normal, outcoming, direct_incoming, roughness);
          ...
          fvec3 direct_in =
              m_scene.m_sun_light->get_component<scene::sun_light>()->energy;
    ...
    if (math::dot(normal, indirect_incoming) > 0) {
      ...
      // Calculate specular BRDF
      float specular_pdf =
          pbr::pdf_specular(normal, outcoming, indirect_incoming, roughness);
      fvec3 specular_brdf(specular_pdf);
      ...
      current_ray =
          geometry::ray(result.position + indirect_incoming * math::epsilon,
                        indirect_incoming);

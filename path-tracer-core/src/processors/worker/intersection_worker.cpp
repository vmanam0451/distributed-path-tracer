  ...
  if (result.hit) {
    ...
    if (sun_light) {
      fvec3 direct_incoming =
          sun_light->get_global_transform().basis * fvec3::backward;
      direct_incoming = util::rand_cone_vec(
          core::rand(),
          math::cos(
              core::rand() *
              sun_light->get_component<scene::sun_light>()->angular_radius),
          direct_incoming);
  ...
  while (!m_should_terminate) {
    ...
    if (!m_object_intersection_results.contains(ray.uuid)) {
      m_object_intersection_results[ray.uuid] =
          std::pair<int, models::cloud_ray>{1, ray};
    }
    ...
    if (results.first < m_worker_info.num_workers) {
      ...
      if (ray.object_intersect_distance <
          previous_best.object_intersect_distance) {
        m_object_intersection_results[ray.uuid] =
    ...
    if (results.first == m_worker_info.num_workers) {
      ...
      if (best_ray.object_intersect_distance ==

...
namespace util {
	...
	thread_pool::thread_pool(uint32_t thread_count) {
		...
		if (thread_count == 0) {
			std::cout << "std::thread::hardware_concurrency() is not supported, falling back to 8 execution threads." <<
				std::endl;
			thread_count = 8;
		}
		...
		while (true) {
			{
				std::unique_lock lock(queue_mutex);
				notifier.wait(lock, [&] {
					return !queue.empty() || terminating.test() || (quit_when_ready != nullptr && quit_when_ready->
						ready());
				});
